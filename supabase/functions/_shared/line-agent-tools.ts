...
import { dateOnlyInBangkok, CUSTOMER_ORDER_URL, LIFF_BOOKING_URL } from "./constants.ts";
...
function formatProducts(products: MatchedProduct[]): string {
  ...
  const line = (p: MatchedProduct) =>
    `- ${[p.brand, p.model].filter(Boolean).join(" ")} ${p.name} (${p.category}): ${p.price.toLocaleString("th-TH")} บาท`;
  const groups: string[] = [];
  ...
  if (bookingOnly.length) groups.push([`กลุ่ม "ต้องเอารถเข้าร้าน" (ลิงก์: ${LIFF_BOOKING_URL}):`, ...bookingOnly.map(line)].join("\n"));
  if (orderOnly.length) groups.push([`กลุ่ม "สั่งผลิตได้เลยไม่ต้องเข้าร้าน" (ลิงก์: ${CUSTOMER_ORDER_URL}):`, ...orderOnly.map(line)].join("\n"));
  if (either.length) groups.push(['กลุ่ม "จองคิวหรือสั่งผลิตก็ได้":', ...either.map(line)].join("\n"));
  return [
...
function formatQueueDensity(queueDays: QueueDay[]): string {
  ...
  const lines = queueDays.map((d) => {
    ...
    return `- ${label}: ${d.is_over_capacity ? "งานเกินกำลังผลิตปกติ (คิวแน่น)" : "ยังไม่เกินกำลังผลิตปกติ"}`;
  });
  return [`สถานะคิว ${queueDays.length} วันข้างหน้า:`, ...lines, `สรุป: ${overCount}/${queueDays.length} วันคิวแน่น`].join("\n");
}
...
export const LINE_AGENT_TOOLS: ToolDef[] = [
  {
    ...
    function: {
      ...
      parameters: {
        type: "object",
        properties: { query: { type: "string", description: "คำค้น เช่น 'แร็คหลังคา Ford Ranger' หรือ 'กันชนหน้า Hilux'" } },
        required: ["query"],
      },
    },
  },
  {
    ...
    function: {
      ...
      description:
        "ตรวจ order/booking ของ LINE user ปัจจุบัน และใช้ผลจาก tool นี้เป็น authoritative source สำหรับชื่อลูกค้า รถ/รุ่น รายการงาน สถานะ และวันกำหนดส่ง/วันนัดที่มีอยู่ในระบบ เมื่อกำลังจะกล่าวถึงข้อมูลของออเดอร์ เช่น รุ่นรถหรือสถานะ ต้องเรียก tool นี้ก่อน ถ้าข้อมูลนั้นไม่ได้ถูกลูกค้าระบุความหมายอย่างชัดเจนในข้อความเดียวกัน ระบบรู้ตัวตนจาก LINE อยู่แล้ว ไม่ต้องมี phone ก็เรียกได้ ใส่ phone เฉพาะตอนที่ลูกค้าพิมพ์เบอร์มาในแชท",
      parameters: {
        type: "object",
        properties: { phone: { type: "string", description: "เบอร์โทรลูกค้าถ้าพิมพ์มาในแชท (10 หลัก)" } },
        required: [],
      },
    },
  },
  {
    ...
    function: {
      name: "check_queue",
      description: "ดูว่าคิวงานผลิตช่วง 7 วันข้างหน้าแน่นแค่ไหน เรียกเมื่อลูกค้าถามว่าตอนนี้คิวยาวไหม/รับงานเพิ่มได้ไหม/สั่งตอนนี้เสร็จเมื่อไหร่",
  ...
  return async (name, args) => {
    switch (name) {
      case "search_products": {
        ...
        const { data, error } = await supabase.rpc("search_products", { customer_message: query });
        if (error) {
      ...
      case "get_order_status": {
        const phone = typeof args.phone === "string" ? args.phone.replace(/[-\s]/g, "").match(/0\d{8,9}/)?.[0] ?? null : null;
        return await getCustomerContext(supabase, lineUid, phone);
      }
      case "check_queue": {
        const { data, error } = await supabase.rpc("get_upcoming_queue_density", { from_date: dateOnlyInBangkok(), days: 7 });
        if (error) {
