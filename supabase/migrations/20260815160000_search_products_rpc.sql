-- ponytail: substring match against brand/model/search_tags/aliases is enough for this catalog size
-- (340 rows, short Thai/English customer messages) — upgrade to full-text search only if this misses often.
create or replace function public.search_products(customer_message text)
returns table (id text, brand text, model text, name text, price integer, category text)
language sql
stable
as $$
  select distinct p.id, p.brand, p.model, p.name, p.price, p.category
