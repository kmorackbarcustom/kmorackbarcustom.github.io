...
- `PLAN-KMO-BK01-ONE-SHOT-INTEGRATION-2026-09-24.md` — single execution worklist above the D1A brief (D1A §F stays semantic authority); `ONE_SHOT_INTEGRATION_FEASIBLE_WITH_GATES`, slice S5 gated by OD-1. Marker: `KMO_BK01_ONE_SHOT_INTEGRATION_PLAN = READY` — planning state only; no implementation, migration, merge, deploy, Order or Claim activation is authorized until an explicit implementation dispatch
